...
namespace UniMod.Context {
    ...
    public sealed class ModClosure : IModClosure {
        ...
        readonly Dictionary<IModLoader, UniTaskCompletionSource<bool>> _loadingOperations;

        Dictionary<string, Mod> _modsById;
        ...
        public void RebuildClosure(IEnumerable<IModLoader> loaders) {
            ...
            if (_modsById is not null) _mods.AddRange(_modsById.Values);
        }

        public IMod GetMod(string id) {
            if (_modsById is null || string.IsNullOrEmpty(id)) return null;
            return _modsById.GetValueOrDefault(id);
        }

        public UniTask<bool> TryLoadAllModsAsync() => WhenAll(_mods.Select(TryLoadModAsync));
        public UniTask<bool> TryLoadModsAsync(params string[] ids) => ids is null ? UniTask.FromResult(false) : WhenAll(ids.Select(TryLoadModAsync));
        public UniTask<bool> TryLoadModsAsync(IEnumerable<string> ids) => ids is null ? UniTask.FromResult(false) : WhenAll(ids.Select(TryLoadModAsync));
        ...
        async UniTask<bool> TryLoadModAsync(Mod mod) {
            // don't load the mod if it has any issues
            if (mod.Issues != 0) return false;
        ...
        static async UniTask<bool> WhenAll(IEnumerable<UniTask<bool>> tasks) {
