...
namespace UniMod.Utilities.Pooling {
    ...
    sealed class Pool<T> where T : class {
        static readonly Action<T> NoOp = delegate { };
        ...
        public Pool(Func<T> createNew) : this(createNew, null, null) { }

        public Pool(Func<T> createNew, Action<T> onGet = null, Action<T> onRelease = null) {
            _stack = new Stack<T>();
            _createNew = createNew ??
                         throw new NullReferenceException("Pool cannot be instantiated with a null createNew delegate");
            _onGet     = onGet ?? NoOp;
        ...
        public IEnumerable<T> Get(int count) {
            for (int i = 0; i < count; ++i) yield return Get();
        }

        public void Get(int count, ICollection<T> collection) {
            if (collection is null) return;
            for (int i = 0; i < count; ++i) collection.Add(Get());
        }
