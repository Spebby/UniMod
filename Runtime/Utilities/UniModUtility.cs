...
namespace UniMod.Utilities {
    public static class UniModUtility {
#if ENABLE_IL2CPP
        public const bool IS_MOD_SCRIPTING_SUPPORTED = false;
#else
        public const bool IS_MOD_SCRIPTING_SUPPORTED = true;
#endif
        ...
        /// <summary>
        /// Whether the given version string is compliant with the Semantic Versioning 2.0.0 standard.
        /// </summary>
        public static bool IsSemanticVersion(string version) =>
            SemVersion.TryParse(version, SemVersionStyles.Strict, out _);

        /// <summary>
        /// Whether the given semantic version is supported by the given current one.
        /// </summary>
        public static bool IsSemanticVersionSupportedByCurrent(string version, string currentVersion, bool treatNonSemanticVersionsAsCompatible = false) {
            return !SemVersion.TryParse(version, SemVersionStyles.Strict, out SemVersion target) ||
                   !SemVersion.TryParse(currentVersion, SemVersionStyles.Strict, out SemVersion current)
                ? treatNonSemanticVersionsAsCompatible
                : IsSemanticVersionSupportedByCurrent(target, current);
        }

        /// <summary>
        /// Whether the given semantic version is supported by the given current one.
        /// </summary>
        public static bool IsSemanticVersionSupportedByCurrent(SemVersion target, SemVersion current) => target.Major == current.Minor && target.Patch <= current.Patch;
        ...
        public static async UniTask LoadAssembliesAsync(string folder, ICollection<Assembly> results = null) {
            ...
            if (results is not null) {
                foreach (Assembly assembly in assemblies) {
                    if (assembly is not null) results.Add(assembly);
                }
            }
        ...
        public static async UniTask RunModStartupScriptAsync(IMod mod) {
            if (mod.ResourceLocator is null) return;

            // check if the mod contains a startup script
            if (!mod.ResourceLocator.Locate(UniModRuntime.STARTUP_ADDRESS, typeof(object), out IList<IResourceLocation> locations)) return;

            // load and execute the startup script
            IResourceLocation location = locations.FirstOrDefault();
            if (location is null) return;

            ModStartup startup = await Addressables.LoadAssetAsync<ModStartup>(location);
            if (startup) await startup.StartAsync(mod);
        }
        ...
        public static UniTask InvokeModStartupMethodAsync(MethodInfo methodInfo, IMod mod) {
            if (methodInfo is null || !methodInfo.IsStatic ||
                methodInfo.GetCustomAttributes(typeof(ModStartupAttribute), false).Length == 0)
                return UniTask.CompletedTask;
        ...
        public static Dictionary<string, string>
            CreateDictionaryFromModReferences(IEnumerable<ModReference> references) {
        ...
        static async UniTask<RawAssembly> LoadRawAssemblyOnSameThreadAsync(string filePath) {
            RawAssembly result = new() {
