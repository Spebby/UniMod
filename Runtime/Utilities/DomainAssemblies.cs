...
namespace UniMod.Utilities {
    ...
    internal static class DomainAssemblies {
        ...
        public static Assembly Load(byte[] rawAssembly, byte[] rawSymbolStore = null) {
            if (rawAssembly is null) throw new NullReferenceException("The given raw assembly is null");
        ...
        /// </summary>
        public static Assembly Get(string fullName) => LoadedAssemblies.TryGetValue(fullName, out Assembly assembly) ? assembly : null;

        /// <summary>
        /// Whether or not the given assembly is loaded on the current AppDomain.
        /// </summary>
        public static bool Contains(Assembly assembly) => LoadedAssemblies.ContainsKey(assembly.FullName);

        /// <summary>
        /// Whether or not the given assembly full name is loaded on the current AppDomain.
        /// </summary>
        public static bool Contains(string assemblyFullName) => LoadedAssemblies.ContainsKey(assemblyFullName);
    }
}
