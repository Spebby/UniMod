...
namespace UniMod.EmbeddedMods {
    ...
    public sealed class EmbeddedModAssetsLocator : IResourceLocator {
        ...
        readonly Dictionary<object, List<IResourceLocation>> _locationsByKey;
        readonly List<object> _keys;
        readonly Dictionary<(object Key, Type Type), IList<IResourceLocation>> _locateCache;

        public static async UniTask<EmbeddedModAssetsLocator> CreateAsync(
            string id, IEnumerable<EmbeddedModAsset> assets) {
            EmbeddedModAssetsLocator locator = new(id);
            await locator.InitializeAsync(assets);
            return locator;
        }

        EmbeddedModAssetsLocator(string id) {
        ...
        public bool Locate(object key, Type type, out IList<IResourceLocation> locations) {
            ...
            (object key, Type type) cacheKey = (key, type);
        ...
        async UniTask InitializeAsync(IEnumerable<EmbeddedModAsset> assets) {
        ...
        async UniTask LoadAssetAsync(EmbeddedModAsset asset) {
            AsyncOperationHandle<IList<IResourceLocation>> handle;
            using Pool<List<object>>.Handle                _ = ListPool<object>.Get(out List<object> keys);
            keys.Add(asset.guid);

            try {
                handle = Addressables.LoadResourceLocationsAsync(keys, Addressables.MergeMode.Union);
                await handle;
            } catch (Exception) {
                return;
            }
        ...
        void AddLocation(object key, IResourceLocation location) {
        ...
        void AddDependencies(IResourceLocation location) {
            if (location.Dependencies is null) return;
