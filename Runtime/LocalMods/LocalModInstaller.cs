...
namespace UniMod.LocalMods {
    ...
    public sealed class LocalModInstaller : ILocalModInstaller {
        ...
        public async UniTask DownloadAndInstallModsAsync(IEnumerable<string> modUrls, CancellationToken cancellationToken = default) {
            using Pool<List<UniTask>>.Handle _ = ListPool<UniTask>.Get(out List<UniTask> tasks);
        ...
        public async UniTask DownloadAndInstallModAsync(string modUrl, CancellationToken cancellationToken = default, IProgress<float> progress = null) {
            if (string.IsNullOrEmpty(modUrl)) throw new Exception("Null or empty mod URL");

            UnityWebRequest request = await UnityWebRequest.Get(modUrl)
                                                           .SendWebRequest()
                                                           .ToUniTask(cancellationToken: cancellationToken, progress: progress);
        ...
        public UniTask InstallModsAsync(string folderPath, bool deleteModFilesAfter = false) {
            if (string.IsNullOrEmpty(folderPath)) throw new Exception("Null or empty folder path");
        ...
        public async UniTask InstallModAsync(byte[] modBuffer) {
            if (modBuffer is null) throw new Exception("Null mod buffer");
            ...
            try {
                ...
                using MemoryStream memoryStream       = new(modBuffer);
                ...
                if (string.IsNullOrEmpty(modId)) throw new Exception("Couldn't fetch the mod ID from the mod archive");
                ...
                if (Directory.Exists(installationPath)) IOUtils.DeleteDirectory(installationPath);
