...
namespace UniMod.LocalMods {
    ...
    public sealed class LocalModLoader : IModLoader {
        ...
        readonly string _assembliesFolder;
        readonly string _catalogPath;
        readonly List<Assembly> _loadedAssemblies;

        UniTaskCompletionSource _loadOperation;
        UniTaskCompletionSource<Sprite> _thumbnailOperation;

        public LocalModLoader(string modFolder, ModInfo info, string source = LocalModSource.SourceLabel) {
            ModFolder = modFolder;
            _assembliesFolder = Path.Combine(modFolder, UniModRuntime.ASSEMBLIES_FOLDER);
            _catalogPath = Path.Combine(modFolder, UniModRuntime.ASSETS_FOLDER, UniModRuntime.ADDRESSABLES_CATALOG_FILE_NAME);
        ...
        public async UniTask<Sprite> GetThumbnailAsync() {
            if (_thumbnailOperation is not null) return await _thumbnailOperation.Task;
        ...
        async UniTask InternalLoadAsync(IMod mod) {
            if (IsLoaded) return;
            if (ContainsAssemblies) await UniModUtility.LoadAssembliesAsync(_assembliesFolder, _loadedAssemblies);
            if (ContainsAssets) ResourceLocator = await Addressables.LoadContentCatalogAsync(_catalogPath, true);
        ...
        async UniTask<Sprite> LoadThumbnailAsync() {
            string path = Path.Combine(ModFolder, UniModRuntime.THUMBNAIL_FILE);
