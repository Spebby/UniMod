...
Originally crated by Pablo Gutierrez Millares.

This is a fork maintained by Spebby for the latest Unity versions.

A Unity package that adds modding support to your project.

- **No content restrictions**. Mods can contain **assets** and **C# scripts**. _Scripts can be disabled_
...
1. UniMod uses UniTask for all async operations, so you will need to [install it first](https://github.com/Cysharp/UniTask#install-via-git-url). _Note: I'm not the author of UniTask_.
2. Install UniMod by using the the following Git URL in the Package Manager (just like you did for UniTask):

   ```
   https://github.com/Spebby/UniMod.git
   ```

3. If you want to enable scripting, you must make sure that your project is configured with the [Mono](https://docs.unity3d.com/Manual/Mono.html) scripting backend in your _Player Settings_. Go to `Edit > Project Settings...`, select `Player` and find the _Scripting Backend_ option. Make sure that **Mono** is set:
...
- **Host Id**: in UniMod, an application that can load mods is called the _Host_, and it should declare an _ID_ and _Version_ so mods can properly target it. It is recommended to follow the `com.company.name` structure.
